...
export default function IndexJobType() {
	...
	function nextSlide() {
		...
		setTransValue(cardWidth / 5.1 * currIndex);
	}
	return (
		<div className="w-full flex overflow-hidden self-center justify-center">
			<div
				className="lg:w-8/9 md:w-8/9 w-full flex lg:flex-row flex-col self-center rounded relative mb-24 pt-12 pb-12 rounded lg:px-12 md: px-8 px-4 overflow-hidden"
				style={{ backgroundColor: '#f7f7f7' }}
				ref={cardRef}
			>
				<div className="lg:w-1/4 pr-4 lg:m-0 mt-8 mb-8">
					<div className="pb-2">
						<p className="lg:text-3xl md:text-3xl text-2xl font-semibold font-sans">
							Solutions designed for all your needs
						</p>
					</div>
					<div className="py-2">
						<p className="leading-loose lg:text-base md:text-base text-sm ">
							Workspace is our craft. From private offices to whole headquarters, we create spaces that
							work for you.
						</p>
					</div>
				...
				<div className="inline-block whitespace-no-wrap flex lg:w-5/6 overflow-hidden">
					...
					>
						{[0, 1, 2, 3, 4, 5, 6, 7].map(id => (
							<motion.div className="lg:h-full h-32">
								<motion.div
									style={{ width: `calc(${cardWidth}px / 5.1)` }}
									className="flex-grow bg-white rounded border lg:px-4 mx-2 hover:shadow cursor-pointer flex items-center justify-center h-full"
									key={id}
